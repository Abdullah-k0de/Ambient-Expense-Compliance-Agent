...
def test_pubsub_trigger_normalization(server_fixture: subprocess.Popen[str]) -> None:
    """
    Test the Pub/Sub trigger endpoint (/apps/expense_agent/trigger/pubsub) with
    a fully-qualified subscription name, and verify that the middleware
    correctly normalizes it to the short subscription name.
    """
    import base64
    ...
    expense_bytes = json.dumps(expense_payload).encode("utf-8")
    expense_b64 = base64.b64encode(expense_bytes).decode("utf-8")

    trigger_payload = {
        "message": {
            "data": expense_b64,
            "messageId": "123456",
            "publishTime": "2026-06-18T03:51:33Z"
        },
        "subscription": "projects/test-project/subscriptions/expense-sub"
    }

    # Trigger endpoint
    trigger_url = f"{BASE_URL}/apps/expense_agent/trigger/pubsub"
    response = requests.post(trigger_url, json=trigger_payload, headers=HEADERS, timeout=15)
    assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"

    # Verify that the session was created under user_id 'expense-sub'
    sessions_url = f"{BASE_URL}/apps/expense_agent/users/expense-sub/sessions"
    sessions_response = requests.get(sessions_url, timeout=10)
    assert sessions_response.status_code == 200
    sessions = sessions_response.json()
    assert len(sessions) > 0, "Expected at least one session to be created for 'expense-sub'"
